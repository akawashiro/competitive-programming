...
- p113区間内の素数の個数 $[2,\sqrt{b})$の素数の表を持っておけば$[a,b)$の素数を求めることができる。
- p107線分上の格子点の個数 最大公約数を使う
- p105Layout
  両辺に一つづつしか変数が現れない不等式を最短経路問題に変換する。
- p85食物連鎖
  各動物$i$について動物$i$が種類Aである、Bである、Cであるの3種類の頂点を作りUnionFindで条件の妥当性を検証する
- p67重複組み合わせ（個数制限あり) DPを高速化するやつ
- p66分割数(n個の区別できない品物をm個の箱に入れるやり方) [写像12相](https://mathtrain.jp/twelveway)を参照
- 蟻本p63 最長増加部分列問題
