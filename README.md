...
# 過去問の復習
## [D - No Need](https://beta.atcoder.jp/contests/abc056/tasks/arc070_b) 内部でDPする二分探索
まずa_i<a_jのとき、aiが必要ならばa_jも必要である。つまり単調性が成り立つ。  
この性質を用いると二分探索ができる。a_iが必要かどうかは和がSであって、K<=a_i+S<K+a_iとなる部分集合が存在するかどうかを判定すればよい。   
これはナップサック問題である。    
